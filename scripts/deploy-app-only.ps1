...
Write-Host "Preparing app-only deployment (ragQueryStack + ragStack)..." -ForegroundColor Cyan
...
try {
    Write-Host "Deploying ragQueryStack..." -ForegroundColor Cyan
    Invoke-CheckedCommand -Command "npx" -Arguments @("cdk", "deploy", "ragQueryStack", "--require-approval", "never") -FriendlyName "CDK deploy ragQueryStack"

    Write-Host "Deploying ragStack..." -ForegroundColor Cyan
    Invoke-CheckedCommand -Command "npx" -Arguments @("cdk", "deploy", "ragStack", "--require-approval", "never") -FriendlyName "CDK deploy ragStack"
}
finally {
    Pop-Location
}
...
Write-Host "App-only deployment completed." -ForegroundColor Green
